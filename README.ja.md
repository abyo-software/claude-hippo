...
**v0.5.0** (2026-05-10 release): production-ready。SHODH OpenAPI v1.0.0 全 14 endpoint 実装、candle-rs native prediction-loss 統合、Hebbian + clustering 完全。

| Bench | 効果 | 詳細 |
|---|---|---|
| A: Long-session noise | precision@1 を **8% → 100%** (既定 oversample=6) | 100 items, 25 paraphrased queries |
| B: Cross-session retrieval (forgetting curve) | 365 日決定が **負の lift → +0.875 lift** (`decay_floor=0.5`) | 50 items × 4 ages |
| C: Decision trace | recall@5 を **0.44 → 1.00** (`--surprise-weights "0.2,0.1,0.5,0.2"`) | 4 Decisions in 20 mixed |
| D: Prediction-loss wiring | mock 100/100 coverage、real candle-local は `#[ignore]` | Qwen2.5-0.5B CPU で実 NLL gradient |

**129 tests** pass + 5 ignored (real-backend gates)。clippy + fmt clean。詳細は [docs/SURPRISE_SELECTION.md](docs/SURPRISE_SELECTION.md)、変更履歴は [CHANGELOG.md](CHANGELOG.md)。
...
```bash
cargo test                                          # default 129 pass + 3 ignored
cargo test --features candle                        # 132 pass + 5 ignored
cargo test --features candle -- --ignored           # real-backend variants (要 1GB DL)
...
コントリビュート歓迎: [CONTRIBUTING.md](CONTRIBUTING.md) / セキュリティ報告: [SECURITY.md](SECURITY.md)
